      ...
      if (paramType === "Boolean") {
        ...
        if (val.match(',')) {
          ...
          if(nearParams.length > 1){
            var lat = nearParams[0]
            var lng = nearParams[1]
          ...
          if(nearParams.length > 2 && !isNaN(nearParams[3]){
            geoFilter.$maxDistance = parseFloat(nearParams[3]) / 3959
          }
    ...
    var parseParam = function (key, val) {
      ...
      if (val === "") {
        ...
        if(parsedVal !== NaN){
          ...
          per_page = parsedVal > 1 ? parsedVal : per_page
        }
