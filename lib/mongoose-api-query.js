  ...
  schema.statics.apiQueryParams = function(rawParams) {
    ...
    var parseSchemaForKey = function(schema, keyPrefix, lcKey, val, ƒ) {
      ...
      if (paramType === "Boolean") {
        ...
        if (nearParams.length > 2 && !isNaN(nearParams[2])) {
          var maxDistance = parseFloat(nearParams[2])
          maxDistance = maxDistance > 0 || 10;
          geoFilter.$near.$maxDistance = (maxDistance / 3959)
        } else {
          geoFilter.$near.$maxDistance = 10 / 3959;
        }
